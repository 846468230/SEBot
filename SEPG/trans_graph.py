...
from utils import load_graph, PartitionTree
...
def update_depth(tree):
    ...
    while wait_update:
        ...
        wait_update = set(
            [tree[nid].parent for nid in wait_update if tree[nid].parent])
...
def pool_trans_discon(input_):
        ...
        if len(sub_nodes) == 1:
            ...
            for d in range(1, tree_depth + 1):
                js.append({
                    ...
                    '%s_%s_0' % (gi, d),
                    'parent':
                    '%s_%s_0' % (gi, d + 1) if d < tree_depth else None,
                    'depth':
                    d,
                    'children': [js[-1]['ID']]
                })
            ...
            for j in js:
                j['ID'] = rmap[j['ID']]
                j['parent'] = rmap[
                    j['parent']] if j['depth'] < tree_depth else None
                j['children'] = [rmap[c] for c in j['children']
                                 ] if j['children'] else None
        trees.append(js)
    id_map = {}
    ...
    for js in trees:
        for j in js:
            ...
            n['children'] = [id_map[c]
                             for c in n['children']] if n['children'] else None
            n['ID'] = id_map[n['ID']]
            tree[n['ID']] = n
            if n['parent'] is None:
...
def struct_tree(dataset='twibot-20', tree_depth=3):
    if not os.path.exists('subgraphs'):
        os.makedirs('subgraphs')
    if os.path.exists('subgraphs/%s_%s.pickle' % (dataset, tree_depth)):
        return
    print('loading subgraphs')
    g_list = load_graph(
        dataset
    )  # load networkx graph list: [{'G': nx.Graph, 'label': int}, ...]
    print('subgraph number: %s' % len(g_list))

    pool_func = pool_trans
    pool = Pool()  # use multi-process pool for speed up
    ...
    with open('subgraphs/%s_%s.pickle' % (dataset, tree_depth), 'wb') as fp:
        pickle.dump(list(g_list), fp)


if __name__ == '__main__':
    for tree_depth in range(3, 9):
        dataset = 'twibot-20'
