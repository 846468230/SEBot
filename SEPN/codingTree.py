...
def graph_parse(adj_matrix):
    ...
    adj_table = {}  # {node_id: set(adj_node_id)}
    VOL = 0  # total volume
    node_vol = []  # [node_volume]
...
def merge(new_ID, id1, id2, cut_v, node_dict):
    ...
    g = node_dict[id1].g + node_dict[
        id2].g - 2 * cut_v  # 这里的意思是当你把两个节点合并成一个新的节点，他在原来图上的度就是两个节点的度之和减去两个节点之间的边的度
    child_h = max(
        node_dict[id1].child_h, node_dict[id2].child_h
    ) + 1  # child_h=0 first, represent the height of the tree rooted at this node, 最下面一层为0，往上一层为1，以此类推
...
def CombineDelta(node1, node2, cut_v, g_vol):
    # node1, node2: PartitionTreeNode
    # cut_v: cut_volume, volume of edges between node1 partition and node2 partition
    # g_vol: total volume of graph
    v1 = node1.vol + 1
    v2 = node2.vol + 1
    g1 = node1.g + 1
    g2 = node2.g + 1
...
class PartitionTreeNode():
    ...
    def __init__(self,
        ...
        self.vol = vol
        self.g = g  # initial the same to vol
        self.merged = False
        self.child_h = child_h  # 该节点的子树的高度
...
class PartitionTree():
    ...
    def __init__(self, adj_matrix):
        ...
        self.g_num_nodes, self.VOL, self.node_vol, self.adj_table = graph_parse(
            adj_matrix)
        # node_num, total volume, node volume list, adj_table: {node_id: set[adj_node_id]}
        self.id_g = get_id(
        )  # start from 0, assign ID to each node, a iterator
        self.leaves = []  # [node_id], initially, all nodes are leaves
        self.build_leaves()

    def build_leaves(self):
        for vertex in range(self.g_num_nodes):
            ID = next(self.id_g
                      )  # assign ID to each node, also for newly merged node
            v = self.node_vol[vertex]
            leaf_node = PartitionTreeNode(
                ID=ID,
                partition=[vertex],  # partition is a list of node_id initially
                g=v,  # g is the volume of the node initially
                vol=v)
    ...
    def __build_k_tree(
            self,
            g_vol,  # total volume of the graph
            nodes_dict: dict,  # {node_id: PartitionTreeNode}
        ...
        for i in nodes_ids:
            # 如果是孤立节点，这里啥也不会做
            for j in self.adj_table[i]:  # j is the id of i's neighbor
                if j > i:  # avoid duplicate
                    n1 = nodes_dict[i]  # PartitionTreeNode of node i
                    n2 = nodes_dict[j]  # PartitionTreeNode of node j
                    # 计算两个节点所属两个社区之间的边数
                    if len(n1.partition) == 1 and len(n2.partition) == 1:
                        cut_v = self.adj_matrix[n1.partition[0], n2.partition[
                            0]]  # cut volume between node i and node j
                    else:
                        cut_v = cut_volume(self.adj_matrix,
                                           p1=np.array(n1.partition),
                                           p2=np.array(n2.partition))
                    diff = CombineDelta(
                        nodes_dict[i], nodes_dict[j], cut_v,
                        g_vol)  # calculate the structure entropy delta
        ...
        while unmerged_count > 1:
            ...
            # if two nodes have been merged, skip
            ...
            self.adj_table[new_id] = self.adj_table[id1].union(
                self.adj_table[id2]
            )  # new node's neighbors are the union of two nodes's neighbors
            for i in self.adj_table[new_id]:
                self.adj_table[i].add(
                    new_id)  # add new node to its neighbors' neighbors
            # compress delta
            # 合并的节点并不是叶节点，所以需要计算新的delta，并放入到cmp_heap中
            ...
            # 这里是对新节点的与他的邻居进行合并，计算新的delta，并放入到min_heap中
        ...
        root = new_id  # 最终所有节点合并到一个节点，这个节点就是根节点
        ...
        if unmerged_count > 1:
            ...
            unmerged_nodes = {
                i
                for i, j in nodes_dict.items() if not j.merged
            }  # 所有未合并的节点id
        ...
        if k is not None:
            while nodes_dict[root].child_h > k:
                diff, node_id, p_id = heapq.heappop(
                    cmp_heap)  # 之前保留的合并子树的id和父节点id
                if child_tree_deepth(nodes_dict, node_id) <= k:  # 节点所处的深度
                    continue
