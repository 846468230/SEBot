...
def extract_layer_data(T, G, tree_depth):
    ...
    for i, n in T.items():
        ...
        interLayerEdges[n['depth']].extend([(n_idx, c - c_base)
                                            for c in n['children']])
...
def load_tree(dataname, tree_depth=2):
    ...
    if dataname == 'twibot-20':
        x = torch.load(path + 'cat_properties_tensor.pt')
    else:
    ...
    if not os.path.exists('trees'):
        os.makedirs('trees')
    with open(
            os.path.join(PWD, 'trees',
                         '%s_%s.pickle' % (dataname, tree_depth)), 'wb') as fp:
